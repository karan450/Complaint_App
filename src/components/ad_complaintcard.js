...
export default function Adcomplaintcard(props){
    ...
    function onhold(){
        axios.post("http://localhost/api/onhold.php",{
            table : props.table,
            status : 'onhold',
            id : props.id
        })
    ...
    function deletecomplaint(){
        axios.post("http://localhost/api/delete.php",{
            table : props.table,
            status : 'inprogress',
            id : props.id
        })
    ...
    return(
        ...
        style={{marginBottom:"20px"}}>
            <div className="header_complaint">
                ...
                <p><FontAwesomeIcon icon={faPhone} /> {props.phoneno}</p>
            ...
            <div className="body_complaint">
                ...
                {props.usertime ? 'Problem Since : '+props.usertime : 
                ...
                style={{
                    ...
                    right: '0', bottom: '0', marginBottom: '20px',
                    marginRight: '20px'
                }}
                // onClick={handleImage}
                 /> : ''}
                ...
                <button className="onhold" onClick={onhold}>On HOLD</button>
                <button className="worker" onClick={sendtoworker}>Send To Worker</button></>:
                // button of on hold page
                props.status=="onhold" ? <button className="worker" onClick={sendtoworker}>Send to WORKER</button>: 
                // button of in progress page
                props.status == "inprogress" ? <button className="worker" onClick={resolved}>RESOLVE</button>:
                props.status == "resolved"?
                // button of resolved page
                <button className="onhold" onClick={deletecomplaint}>DELETE</button> : ''}
                </>
                }

                <span className="ad_logo" 
                style={toggle ? {marginBottom: "12px",marginLeft:"10px",marginTop:"30px"} 
                : 
                { marginBottom: "12px",marginLeft: "10px"}}
                onClick={handleShowmore} 
                ref={showmore}>(Show more)</span>
                </div>
